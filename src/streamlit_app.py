...
.main-title { font-family: 'Inter', sans-serif; color: #1a1a1a; font-size: 26px; font-weight: 700; }
.sub-title { font-family: 'Inter', sans-serif; color: #6b7280; font-size: 13px; }
.section-title {
    font-family: 'Inter', sans-serif; color: #111827; font-size: 13px;
    font-weight: 600; letter-spacing: 0.05em; text-transform: uppercase;
    border-left: 3px solid #f97316; padding-left: 10px; margin: 20px 0 12px;
}
</style>
""",
...
with st.sidebar:
    st.markdown("### 🛠 系统设置")
    mode = st.radio("功能模式", ["🤖 AI智能分析", "📊 量化回测", "🔍 板块筛选"])
    st.markdown("---")
...
if mode == "🤖 AI智能分析":
    ...
    with col2:
        if analyze_btn and stock_code:
            ...
            try:
                resp = requests.post(
                    f"{API_BASE}/analyze", json={"stock_code": stock_code}, timeout=300
                )
