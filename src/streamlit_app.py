...
.stButton > button { background:#111 !important; color:white !important; border:none !important; border-radius:8px !important; font-weight:500 !important; font-size:12px !important; padding:7px 16px !important; }
...
with col_left:
    ...
    <div class="model-selector">
        <div class="model-selector-label">分析模型</div>
    </div>
    """, unsafe_allow_html=True)

    for mk, mv in MODEL_OPTIONS.items():
        is_active = cur_model == mk
        check = "✓" if is_active else ""
        active_cls = "active" if is_active else ""
        st.markdown(f"""
        <div class="model-btn {active_cls}">
            <span class="model-icon">{mv['icon']}</span>
            <div class="model-info">
                <div class="model-name">{mv['label']}</div>
                <div class="model-desc">{mv['desc']}</div>
            </div>
            <span class="model-check">{check}</span>
        </div>
        """, unsafe_allow_html=True)
        if st.button(mv["label"], key=f"model_{mk}", use_container_width=True):
    ...
    for nav_key, icon, label in [
        ...
        if st.button(f"{icon}  {label}", key=f"navbtn_{nav_key}", use_container_width=True):
            st.session_state.mode = nav_key; st.rerun()
    ...
    if convs:
        ...
        st.markdown('<div style="font-size:11px;color:#ccc;padding:4px">暂无历史</div>', unsafe_allow_html=True)

    st.markdown("---")
    pill_cls = "status-online" if backend_ok else "status-offline"
    pill_txt = "● 服务正常" if backend_ok else "● 服务异常"
    st.markdown(f'<div class="status-pill {pill_cls}">{pill_txt}</div>', unsafe_allow_html=True)
...
with col_right:
    ...
    if mode == "chat":
        ...
        for msg in messages:
            if msg["role"] == "user":
                ...
                if mtype == "analysis":
                    ...
                    t1, t2, t3, t4, t5 = st.tabs(["💼 决策", "📊 基本面", "📈 技术面", "📰 情绪", "🔬 辩论"])
                    with t1: st.markdown(dtxt)
                    with t2: st.markdown(data.get("fundamental_report", "暂无"))
                    with t3: st.markdown(data.get("technical_report", "暂无"))
                    with t4: st.markdown(data.get("sentiment_report", "暂无"))
                    with t5: st.markdown(data.get("researcher_analysis", "暂无"))
        ...
        if st.session_state._pending_code:
            ...
            with st.spinner(f"正在用「{MODEL_OPTIONS[model]['label']}」模式分析 {code}..."):
                try:
                    resp = requests.post(
                        f"{API_BASE}/analyze",
                    ...
                    if resp.status_code == 200:
                        ...
                        st.session_state.conversations[cid]["messages"].append({
                            ...
                            "content": "", "data": data, "model": model,
                        })
                        d = data.get("decision", "")
                        action = "买入" if "买入" in d else ("减仓" if ("减仓" in d or "卖出" in d) else "观望")
        ...
        if st.session_state.uploaded_file or st.session_state.uploaded_image:
            ...
            if st.session_state.uploaded_image:
                fname = st.session_state.uploaded_image.name
            elif st.session_state.uploaded_file:
                fname = st.session_state.uploaded_file.name
            st.markdown(f'<div class="upload-preview">📎 {fname} <span style="cursor:pointer;color:#94a3b8" onclick="">✕</span></div>', unsafe_allow_html=True)
        ...
        with btn_col:
            send = st.button("发送", key="send_btn")
        ...
        with tc3:
            ...
            <div class="model-pill">
                {MODEL_OPTIONS[cur_m]['icon']} {MODEL_OPTIONS[cur_m]['label']}
            </div>
            """, unsafe_allow_html=True)
        ...
        if send and user_input.strip():
            ...
            if not st.session_state.current_conv:
                new_conv()
            cid = st.session_state.current_conv
            has_file = bool(st.session_state.uploaded_file or st.session_state.uploaded_image)
            file_name = ""
            if st.session_state.uploaded_image:
    ...
    # ── 量化回测 ──
        ...
        with c1: code = st.text_input("股票代码", "600150", key="bt_code")
        with c2:
            strat = st.selectbox("策略", ["kdj_oversold","j_extreme","rsi","boll","kdj_macd"],
                format_func=lambda x: {"kdj_oversold":"KDJ超卖","j_extreme":"J极值","rsi":"RSI超卖","boll":"布林带","kdj_macd":"KDJ+MACD"}[x])
        with c3: sd = st.text_input("开始", "20240101")
        with c4: ed = st.text_input("结束", "20260530")

        if st.button("开始回测 →"):
            with st.spinner("回测中..."):
                try:
                    resp = requests.post(f"{API_BASE}/backtest",
                        json={"stock_code":code,"strategy":strat,"start_date":sd,"end_date":ed,"initial_cash":100000},
                        timeout=120)
                    if resp.status_code == 200:
                        st.session_state.bt_result = resp.json(); st.rerun()
                    else:
        ...
        if bt:
            ...
            for col, (lbl, val, cls) in zip(cols, [
                ...
                with col:
                    st.markdown(f'<div class="mini-metric"><div class="mini-metric-label">{lbl}</div><div class="mini-metric-value {cls}">{val}</div></div>', unsafe_allow_html=True)

            if bt.get("returns_data") and bt.get("dates_data"):
                cum = ((1 + pd.Series(bt["returns_data"], index=bt["dates_data"])).cumprod() - 1) * 100
                fig = go.Figure()
                fig.add_trace(go.Scatter(x=cum.index, y=cum.values, mode="lines",
                    line=dict(color="#111827", width=1.6), fill="tozeroy", fillcolor="rgba(17,24,39,0.04)"))
                ...
                fig.update_layout(paper_bgcolor="white", plot_bgcolor="white",
                    ...
                    height=220, margin=dict(l=44,r=16,t=8,b=32),
            ...
            if bt.get("trade_records"):
                ...
                for i in range(min(len(buys), len(sells))):
                    ...
                    pnl = (sp - bp) / bp * 100
                    pairs.append({"#": i+1, "买入": buys.iloc[i]["date"], "买入价": f"¥{bp}",
                        "卖出": sells.iloc[i]["date"], "卖出价": f"¥{sp}", "盈亏": f"{pnl:+.2f}%",
                        "": ("✓" if pnl > 0 else "✗")})
                if pairs:
                    st.dataframe(pd.DataFrame(pairs), use_container_width=True, hide_index=True)
            st.caption(bt.get("report_text", ""))
        else:
            st.markdown('<div style="text-align:center;padding:50px;color:#9ca3af;font-size:13px">填写参数，点击「开始回测」</div>', unsafe_allow_html=True)

    # ── 今日买点 ──
    elif mode == "scan":
        st.markdown('<div class="page-title">今日买点</div>', unsafe_allow_html=True)
        st.markdown('<div class="page-sub">全市场扫描 · KDJ超卖 + MA20趋势 · Agent验证 · 市值≥300亿</div>', unsafe_allow_html=True)

        c1, c2 = st.columns([2, 1])
        with c1: bs = st.text_input("数据起始日期", "20230101")
        with c2: tn = st.slider("最多显示", 5, 20, 10)

        if st.button("开始扫描 →"):
            with st.spinner("扫描中（约5分钟）..."):
                try:
                    resp = requests.post(f"{API_BASE}/scan/today",
                        json={"base_start": bs, "top_n": tn}, timeout=600)
                    if resp.status_code == 200:
                        data = resp.json()
                        recs = data.get("recommendations", [])
                        st.success(f"扫描完成 · 候选 {data['total_candidates']} 只 · 推荐 {data['count']} 只")
                        for r in recs:
                            icon = "🔴" if r["confidence"] == "高" else "🟡"
                            with st.expander(f"{icon} {r['name']}（{r['code']}）— {r['decision']} · 置信度:{r['confidence']}"):
                                ca, cb = st.columns([1, 2])
                                with ca: st.metric("当前价", f"¥{r['close']}")
                                with cb: st.markdown(r["report"])
                    else:
    ...
    # ── 板块筛选 ──
        ...
        st.markdown('<div class="page-sub">主题景气周期 · PE / ROE / 毛利率多维评分</div>', unsafe_allow_html=True)

        c1, c2, c3 = st.columns([2, 1, 1])
        with c1: sector = st.selectbox("板块", list(SECTORS.keys()))
        with c2: min_score = st.slider("最低评分", 50, 90, 65)
        with c3: top_n = st.slider("数量", 3, 10, 5)

        if st.button("开始筛选 →"):
            try:
                stocks = SECTORS.get(sector, {})
                resp = requests.post(f"{API_BASE}/filter",
                    json={"sector": sector, "stocks": stocks, "min_score": min_score, "top_n": top_n},
                    timeout=60)
                if resp.status_code == 200:
                    ...
                    if results:
                        df = pd.DataFrame([{
                            ...
                            "毛利率": f"{r['gross_margin']:.1f}%" if r.get("gross_margin") else "N/A",
                        } for r in results])
                        st.dataframe(df, use_container_width=True, hide_index=True,
                            column_config={"评分": st.column_config.ProgressColumn("评分", min_value=0, max_value=100)})
                    else:
            ...
            st.markdown('<div style="text-align:center;padding:50px;color:#9ca3af;font-size:13px">选择板块，点击「开始筛选」</div>', unsafe_allow_html=True)